# Lietuvių 625 — Art Style Specification (files 1.7.1)
...
- Population follows the setting for ALL nouns (1.7.1): the baseline human in
  any noun inset is a CIVILIAN, not a coverall worker. Coveralls appear only
  where the setting is genuinely a work setting — the Materials class by
  default, plus specific tool/industry rows (engine, machine oil, tool) pinned
  worker in deck_builder. The grammar inset prose no longer hard-codes
  "worker"; it names a neutral person and the engine dress class decides.
- Insets never confuse the meaning (1.7.1): every inset must support the target
  (best), or at least be attractive and on-theme; it must NEVER depict anything
  mistakable for a neighbouring subject. This gate now lives in the prompt
  (go_generator INSET_MEANING_RULE), not only in human QA, to cut re-rolls.
- Style clause is defined ONCE in the GO Generator and never edited per-image;
  only {SUBJECT} and {SCENE}/staging vary
- Record the full prompt of every accepted image in the master CSV ledger. NOTE
  (1.7): on the OpenAI gpt-image-1.5 backend there is NO seed to record (the
  field is blank); the prompt is the reproducibility record. Seeds exist only on
  the Z-Image fallback path.
...
- files 1.7 — the GUESSABILITY pass, from the owner's QA of cards 1-127.
              ...
              for rows 9, 22, 24, 32, 34, 35, 36, 39, 40, 41, 43, 48, 55, 93,
              94, 97, 107, 114, 137, 179, 195, 208 — 22 rows AS SHIPPED; the
              1-127 label understates it, the pass grew to #208 as the same
              failures recurred in food/drink); OVERRIDE_CLASS adds 51/56/101
              attribute, 52 chart, 53 nature. Wordlist 2.3: 22 subject phrases
              sharpened (24, 32, 35, 36, 37, 39, 40, 41, 48, 51, 52, 53, 56, 84,
              88, 97, 98, 101, 104, 106, 111, 124). go_generator UNCHANGED;
              house style untouched. Generation frontier at interruption: cards
              #1-231 complete except #227 (raidė, glyph — deliberately skipped).
              (See files_1_7_DIFF.md for the full corrected diff; the earlier
              draft under-reported NOUN_STAGING by 5 rows.)
- files 1.7.1 — noun-inset dress + inset-meaning gate. go_grammars: noun-inset
              population follows the SETTING like verbs — DEFAULT_PEOPLE flips
              worker -> CIVILIAN, PEOPLE_CLASS adds Materials -> worker, and the
              grammar inset prose de-hard-codes "worker" (neutral "person", dress
              class decides). deck_builder: tool/industry rows pinned worker
              (21 variklis, 195 tepalas, 245 įrankis; editable seed list).
              go_generator: INSET_MEANING_RULE baked into the prompt (insets must
              support recognition, never depict a mistakable neighbour) to move
              the 1.7 guessability gate from human QA into generation and cut
              re-rolls. No wordlist change (still 2.3). Affects future
              generations/re-rolls only; the 190 existing images are unchanged.
